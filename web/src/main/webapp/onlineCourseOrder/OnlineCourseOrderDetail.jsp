		...
		<c:forEach var="detail" items="${order.orderDetailList}">
			<div class="orderPhoto">
				<div>
					...
					<br>
						<c:if test="${!comments[detail.courseNo]}">
						...
						<c:if test="${comments[detail.courseNo]}">
 							<button style="border-radius: 1rem; border: 1px solid #ccc;">
								<a style="text-decoration: none; color: #333;" href='<%=request.getContextPath()%>/MemberOnlineCourseCommentServlet?action=getMember_For_Display'>查看我的評價</a>
  							</button>
  						</c:if>
				</div>
				<div>
